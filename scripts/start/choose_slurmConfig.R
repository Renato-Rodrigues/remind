...
choose_slurmConfig <- function(identifier = FALSE, flags = NULL) {
  ...
  if (slurm) { 
    ...
    comp <- switch(as.integer(identifier),
                    "1" = "--qos=standby --partition=priority --nodes=1 --tasks-per-node=12"  , # SLURM standby  - task per node: 12 (nash H12) [recommended]
                    "2" = "--qos=standby --partition=priority --nodes=1 --tasks-per-node=13"  , # SLURM standby  - task per node: 13 (nash H12 coupled)
                    "3" = "--qos=standby --partition=priority --nodes=1 --tasks-per-node=16"  , # SLURM standby  - task per node: 16 (nash H12+)
                    "4" = "--qos=standby --partition=priority --nodes=1 --tasks-per-node=1 --mem=8000"   , # SLURM standby  - task per node:  1 (nash debug, test one regi)
                    ...
                    "9" = "--qos=short --partition=standard --nodes=1 --tasks-per-node=12"    , # SLURM short    - task per node: 12 (nash H12)
                   "10" = "--qos=short --partition=standard --nodes=1 --tasks-per-node=16"    , # SLURM short    - task per node: 16 (nash H12+)
                   "11" = "--qos=short --partition=standard --nodes=1 --tasks-per-node=1 --mem=8000"     , # SLURM short    - task per node:  1 (nash debug, test one regi)
                   "12" = "--qos=medium --partition=standard --nodes=1 --tasks-per-node=1 --mem=8000"    , # SLURM medium   - task per node:  1 (negishi)
                   "13" = "--qos=long --partition=standard --nodes=1 --tasks-per-node=1 --mem=8000"      , # SLURM long     - task per node:  1 (negishi)
                   "14" = "--qos=medium --partition=standard --nodes=1 --tasks-per-node=12"   , # SLURM medium   - task per node: 12 (nash long calibration)
                   "15" = "--qos=medium --partition=standard --nodes=1 --tasks-per-node=16"   , # SLURM medium   - task per node: 16 (nash long calibration)
                   "16" = "direct")
    if (! exists("wasselect")) {
